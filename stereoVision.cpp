...
std::string left_file = "../data/stereo/left.png";
std::string right_file = "../data/stereo/right.png";
...
int main (int argc, char** argv) {
    ...
    for (int v = 0; v < left.rows; ++v) {
        for (int u = 0; u < left.cols; ++u) {
            ...
            double x = (u - cx) / fx;
            double y = (v - cy) / fy;
            double depth = fx * b / (disparity.at<float>(v, u)); // z = fb/d, d = uL - uR
            point[0] = x * depth;
            point[1] = y * depth;
            point[2] = depth;
...
{
    if (pointcloud.empty()) {
        std::cerr << "Point cloud empty!" << std::endl;
        return;
    }
